    ...
    try:
        ...
        if event.lower() in ("start", "incoming", "call_attempt"):
            ...
            if not wav_path or not os.path.exists(wav_path):
                resp = "<Response><Say>Welcome to Grand Hotel. How can I help you?</Say><Hangup/></Response>"
...
<Response>
  ...
  <Record timeout="5" maxDuration="30"/>
                ...
                return Response(content=resp, media_type="application/xml")
            resp = "<Response><Say>Sorry, something went wrong.</Say><Hangup/></Response>"
            return Response(content=resp, media_type="application/xml")

        elif event.lower() in ("completed", "hangup", "end"):
            resp = "<Response><Say>Thank you for calling. Goodbye.</Say></Response>"
            return Response(content=resp, media_type="application/xml")

        # Default fallback
        resp = "<Response><Say>Thank you for calling.</Say><Hangup/></Response>"
        return Response(content=resp, media_type="application/xml")

    except Exception as e:
        logger.error(f"Webhook error: {e}")
        resp = "<Response><Say>Sorry, an error occurred.</Say><Hangup/></Response>"
        return Response(content=resp, media_type="application/xml")
