...
def process_booking_tool(entities: dict, user_phone: str) -> str:
    ...
    if not avail: return f"Sorry, no {room_type or ''} rooms available now."
    if guest_name and check_in and check_out:
        ...
        else: return "Booking failed. Try again."
    missing=[]
...
def process_food_order_tool(entities: dict, user_phone: str) -> str:
    ...
    if not bookings: return "No booking found. Please provide your room number."
    booking_id = bookings[0]["id"]
    room_number = bookings[0]["rooms"]["room_number"]
    if not items: return get_food_menu_and_voice()
    total = 0
    for item in items:
